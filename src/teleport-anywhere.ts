// stolen from dmitmel's crosscode-tweak-pack, converted to typescrit and added features
// https://github.com/dmitmel/crosscode-tweak-pack/blob/master/src/teleport-anywhere.js
...
declare global {
    namespace sc {
        interface MapAreaContainer {
            mouseWasOverWhenDragged: boolean
        }
    }
}
...
prestart(() => {
    sc.MapAreaContainer.inject({
        ...
        onMouseInteract(mouseOver, mouseClicked) {
            ...
            const hoverRoom = this.hoverRoom as sc.AreaRoomBounds
            let hoveredMapName = hoverRoom.text
            let hoveredMapID = hoverRoom.name
            // I'll admit I got carried away with formatting.
            let dialogText = [
                ig.lang.grammarReplace('\\c[3]Teleport\\c[0] to map "[!]"?', hoveredMapName),
                ig.lang.grammarReplace('\\c[3]Map ID:\\c[0] [!]', hoveredMapID),
            ].join('\n')
            sc.BUTTON_SOUND.submit.play()

            const destMapName = hoveredMapID!
            const teleport = () => {
                if (Opts.clickToTeleportInstant) {
                    const steps: ig.EventStepBase.Settings[] = [
                        { type: 'TELEPORT', map: destMapName, marker: 'landmark' },
                        // { time: 3, ignoreSlowDown: false, type: 'WAIT' },
                    ]
                    const event = new ig.Event({ steps })
                    event.addHint('SKIN_ALLOWED')
                    sc.map.teleportEvent = event
                    ...
                    sc.map.startTeleport({ path: destMapName })
                }
            }

            if (Opts.clickToTeleportNoQuestion) {
                teleport()
            } else {
