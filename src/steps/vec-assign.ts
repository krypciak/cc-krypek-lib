...
prestart(() => {
    ig.EVENT_STEP.ASSIGN_VEC2 = ig.EventStepBase.extend({
        init(settings) {
            ...
            if (!this.varName) throw new Error('ig.EVENT_STEP.ASSIGN_VEC2 "varName" is missing!')
            if (!this.value) throw new Error('ig.EVENT_STEP.ASSIGN_VEC2 "value" is missing!')
        },
        start() {
            const varName = ig.Event.getVarName(this.varName)
            if (!varName) throw new Error('ig.EVENT_STEP.ASSIGN_VEC2 "varName" is null!')
...
prestart(() => {
    ig.EVENT_STEP.ASSIGN_VEC3 = ig.EventStepBase.extend({
        init(settings) {
            this.varName = settings.varName
            this.value = settings.value
            if (!this.varName) throw new Error('ig.EVENT_STEP.ASSIGN_VEC3 "varName" is missing!')
            if (!this.value) throw new Error('ig.EVENT_STEP.ASSIGN_VEC3 "value" is missing!')
        },
        start() {
            const varName = ig.Event.getVarName(this.varName)
            if (!varName) throw new Error('ig.EVENT_STEP.ASSIGN_VEC3 "varName" is null!')
