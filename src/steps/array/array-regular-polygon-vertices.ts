...
prestart(() => {
    ig.EVENT_STEP.SET_ARRAY_REGULAR_POLYGON_VERTICES = ig.EventStepBase.extend({
        init(settings) {
            ...
            if (!this.varName) throw new Error(`ig.EVENT_STEP.SET_ARRAY_REGULAR_POLYGON_VERTICES "varName" missing!`)
            if (!this.size) throw new Error(`ig.EVENT_STEP.SET_ARRAY_REGULAR_POLYGON_VERTICES "size" missing!`)
            if (!this.radius) throw new Error(`ig.EVENT_STEP.SET_ARRAY_REGULAR_POLYGON_VERTICES "radius" missing!`)
        },
        start() {
            const varName = ig.Event.getVarName(this.varName)
            if (!varName) throw new Error('ig.EVENT_STEP.SET_ARRAY_REGULAR_POLYGON_VERTICES "varName" is null!')
